...
export class PostsController {
  ...
  async searchPostsBody(
    ...
    // Si body.criterias est un tableau, on fusionne les résultats de chaque critère
    if (Array.isArray(body.criterias)) {
      const allResults: any[] = [];
      for (const criteria of body.criterias) {
        const posts = await this.postsService.searchPostsByCriteria(criteria);
        if (Array.isArray(posts)) {
          allResults.push(...posts);
        }
      }
      // Optionnel : dédupliquer par _id si besoin
      const uniqueResults = Array.from(new Map(allResults.map(p => [String(p._id), p])).values());
      return uniqueResults;
    } else {
      // Cas rétrocompatible : un seul critère
      return this.postsService.searchPostsByCriteria(body);
    }
  }
