...
export class PostsService {
  ...
  async deletePost(postId: Types.ObjectId, userId: Types.ObjectId, siteId: Types.ObjectId): Promise<{ message: string }> {
    ...
    // Optionnel : Supprimer les médias associés (fichiers et entrées DB)
    if (post.media && post.media.length > 0) {
      for (const mediaId of post.media) {
        const media = await this.mediaModel.findById(mediaId);
        if (media) {
          // Supprimer le fichier du serveur (nécessite 'fs' et gestion des chemins)
          // Exemple: fs.unlinkSync(path.join(__dirname, '../../uploads', media.filename));
          // Pour l'instant, on supprime juste l'entrée de la base de données
